...
const name = `Repose-Lite-${metadata.version}-mac-${process.arch}`
...
const source = path.join(root, 'src-tauri/target/release/bundle/macos/Repose Lite.app')
...
const app = path.join(release, 'Repose Lite.app')
run('/usr/bin/ditto', [source, app])
const stage = await mkdtemp(path.join(tmpdir(), 'repose-dmg-'))
const dmg = path.join(release, `${name}.dmg`)
try {
  run('/usr/bin/ditto', [app, path.join(stage, 'Repose Lite.app')])
  await symlink('/Applications', path.join(stage, 'Applications'))
  run('/usr/bin/hdiutil', ['create', '-volname', `Repose Lite ${metadata.version}`, '-srcfolder', stage, '-format', 'UDZO', '-fs', 'HFS+', dmg])
  ...
  console.log(`App: ${app}\nDMG: ${dmg}\nLocal ad hoc signing only; not Apple notarized.`)
} finally {
