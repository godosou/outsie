...
export function ensureIdentity() {
  ...
  try {
    ...
    const p12Args = ['pkcs12', '-export', '-inkey', key, '-in', cert, '-out', p12, '-passout', 'pass:outsie', '-name', IDENTITY]
    const importArgs = ['import', p12, '-k', KEYCHAIN, '-P', 'outsie', '-T', '/usr/bin/codesign', '-T', '/usr/bin/security']
