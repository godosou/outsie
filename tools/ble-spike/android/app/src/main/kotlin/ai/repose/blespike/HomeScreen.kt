...
fun buildHomeScreen(
    ...
    val root = screenScaffold(context, pal, title = "", showTitle = false) { column ->
        ...
        toggle = Switch(context).apply {
            // Design token: the track goes green when on, the knob stays
            // light (--green / --knob). The previous colouring -- a green
            // knob on a sage track -- left the on-state track invisible
            // against the white card, so the switch read as a loose dot.
            val knobOff = if (ReposeTheme.isNight(context)) pal.textSecondary else pal.surface
            thumbTintList = ColorStateList(
                arrayOf(intArrayOf(android.R.attr.state_checked), intArrayOf()),
                // Night: onAccent is the page ground there, which put a dark knob on a
                // light-green track. The knob stays light in both modes.
                intArrayOf(if (ReposeTheme.isNight(context)) pal.textPrimary else pal.onAccent, knobOff),
            )
            // Our own track, not a tint of the stock one: the framework's
            // Material track is drawn at 30% alpha, so tinting it green gave a
            // pale pill that looked like an off switch.
            fun pill(color: Int): Drawable = InsetDrawable(
                Ui.rounded(color, context.dpF(10f)).apply { setSize(context.dp(36), context.dp(16)) },
                0, context.dp(2), 0, context.dp(2),
            )
            trackDrawable = StateListDrawable().apply {
                addState(intArrayOf(android.R.attr.state_checked), pill(pal.accent))
                addState(intArrayOf(), pill(pal.ringSoft))
            }
    ...
    fun refresh() {
        ...
        subhead.text = when {
            ...
            // Broken at the semicolon on purpose: at this width the one-line
            // version wrapped with 「车。」 alone on the second line.
            else -> "带着手机走，它们自己锁；\n走回去，按一下回车。"
        }
...
private fun computerCard(
    ...
    fun gated(label: String, primary: Boolean, onClick: () -> Unit): TextView =
        (if (primary) Ui.primaryButton(context, pal, label, onClick) else Ui.ghostButton(context, pal, label, onClick)).apply {
            isEnabled = heard
            alpha = if (heard) 1f else 0.45f
        }
    val actions = LinearLayout(context).apply { orientation = LinearLayout.HORIZONTAL }
    // MATCH_PARENT height: the two buttons have different minimum heights,
    // and side by side they must be the same height.
    actions.addView(
        gated("锁定", primary = false) { sendLock(context) },
        LinearLayout.LayoutParams(0, MATCH_PARENT, 1f),
    )
    actions.addView(
        // The control screen belongs to THIS computer (design doc §04). It is
        // the card's primary button: 锁定 is what happens by itself when you
        // walk away, 控制 is what you opened the app to do.
        gated("控制", primary = true) { controlMac(m.keyId); nav.go(Screen.CONTROL) },
        LinearLayout.LayoutParams(0, MATCH_PARENT, 1f).also { it.leftMargin = context.dp(10) },
    )
    ...
    links.addView(
        TextView(context).apply {
            ...
            text = "重新量距离"
            setTextColor(pal.textSecondary)
            // Dimmed the same way as the two buttons above it. In the divider
            // colour it disappeared, which reads as a link that was never there.
            alpha = if (heard) 1f else 0.45f
