...
fun buildCalScreen(context: Context, nav: Nav, store: AppStore): ScreenView {
    ...
    val root = screenScaffold(
        ...
        // The hero sits under a 25sp title with no lead between; 6dp read as
        // the title touching the card.
        column.addView(body, Ui.lp(top = context.dp(12)))
    }
    ...
    render = fun() {
        ...
        when (step) {
            CalStep.INTRO -> {
                ...
                //
                // One note, not two: what this Mac has (§01, first), then what
                // the measurement needs. Two identical sage blocks in a row
                // read as the same thing said twice.
                body.addView(Ui.infoNote(context, pal,
                    (if (calibrated) "量过一次。重新量的时候，旧的先照常用着；量不出结果，旧的也不会变。"
                    else "这台电脑还没量过。") +
                        "\n\nMac 上要开着 Outsie，屏幕别锁着。量的是 Mac 听到的信号。"), Ui.lp(top = context.dp(12)))
