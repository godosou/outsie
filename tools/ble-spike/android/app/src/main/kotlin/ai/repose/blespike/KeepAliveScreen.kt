...
fun buildKeepAliveScreen(context: Context, store: AppStore, nav: Nav, onRequestPermissions: () -> Unit): ScreenView {
    ...
    val root = screenScaffold(
        context, pal,
        title = "让它一直在",
        lead = "息屏、锁屏、放进口袋，钥匙都要在。这部手机要允许几件事，做过就不用再管。",
        ...
        KeepAlive.items(keep).forEachIndexed { index, item ->
            ...
            // The first card sits under the lead paragraph, not under another
            // card, so it gets the larger gap.
            column.addView(card, Ui.lp(top = context.dp(if (index == 0) 16 else 12)))
        }
        column.addView(
