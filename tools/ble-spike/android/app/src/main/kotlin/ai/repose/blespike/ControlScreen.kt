...
fun buildControlScreen(context: Context, nav: Nav, console: ConsoleServer, store: AppStore): ScreenView {
    ...
    val root = screenScaffold(
        ...
        // Broken at the semicolon: the two clauses are parallel, and on one
        // line the sentence wrapped with 「键。」 alone on the second.
        lead = "点 App 名字，Mac 切过去；\n点按钮，切过去再按键。",
        ...
        if (cat == null || cat.apps.isEmpty()) {
            ...
            // The chip row scrolls edge to edge: the scroller reaches past the
            // column's side padding and the row puts that padding back, so a
            // chip that does not fit is cut at the screen edge -- which reads
            // as 「还有」 -- rather than at the content margin, which read as
            // a broken chip.
            val gutter = context.dp(20)
            // The column clips to its padding by default, which would cut the
            // scroller back to the content margin.
            ...
            column.addView(
                ...
                Ui.lp(top = context.dp(14), left = -gutter, right = -gutter),
            )
...
private fun grid(
    ...
    actions.chunked(3).forEachIndexed { r, rowActions ->
        ...
        rowActions.forEachIndexed { i, act ->
            ...
            wire(act, t)
            // MATCH_PARENT height: a two-line name made one tile taller than
            // its neighbours, and the row read as unfinished.
            row.addView(t, LinearLayout.LayoutParams(0, MATCH_PARENT, 1f).also { if (i > 0) it.leftMargin = context.dp(8) })
        }
        // The spacers fill the height as well: LinearLayout only sizes a
        // wrap-content row to its tallest MATCH_PARENT child when every child
        // is MATCH_PARENT, so a 1px spacer collapsed the row to 1px.
        repeat(3 - rowActions.size) {
            row.addView(View(context), LinearLayout.LayoutParams(0, MATCH_PARENT, 1f).also { it.leftMargin = context.dp(8) })
        }
...
private fun tile(context: Context, pal: Palette, act: ConsoleAction, editing: Boolean, hiddenSection: Boolean): FrameLayout {
    val body = LinearLayout(context).apply {
        ...
        // A fixed box for the glyph: 「↵」 and 「⌘」 are not the same height,
        // and without this each row of tiles came out a different height.
        addView(TextView(context).apply {
            text = act.icon ?: "⌨"
            setTextColor(pal.accent)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 22f)
            gravity = Gravity.CENTER
        }, Ui.lp(height = context.dp(32)))
        addView(TextView(context).apply {
            text = act.name
    ...
    return FrameLayout(context).apply {
        ...
        clipChildren = false
        clipToPadding = false
        // MATCH_PARENT so the body fills a tile stretched to its row's height.
        addView(body, FrameLayout.LayoutParams(MATCH_PARENT, MATCH_PARENT))
    }
}
    ...
    for (r in 0 until g.childCount) {
        ...
        for (i in 0 until row.childCount) {
            ...
            t.addView(
                ...
                // Inside the corner, not hanging off it: the rows and the
                // column above clip their children, so a badge pushed past the
                // tile's edge was drawn as a quarter circle.
...
private fun chip(context: Context, pal: Palette, label: String, on: Boolean, onClick: () -> Unit): TextView =
    TextView(context).apply {
        ...
        // The selected chip gets the darker fill and a green outline
        // (.apptab.on in the design doc). accentSoft and surfaceMuted are the
        // same colour, so before this the only difference was the weight.
        background =
            if (on) Ui.rounded(pal.ringSoft, context.dpF(999f), pal.accent, context.dp(1))
            else Ui.rounded(pal.surfaceMuted, context.dpF(999f))
...
private fun link(context: Context, pal: Palette, label: String, onClick: () -> Unit): TextView =
    TextView(context).apply {
        ...
        // Room to hit: the text is small, the target need not be.
