...
# How long a silence has to last before the countdown to the silence lock is
# shown. Shorter than STALE_S on purpose: the person should see the lock
# coming for a good half minute, not for the last fifteen seconds.
SILENT_NOTICE_S="${REPOSE_SILENT_NOTICE_S:-20}"
last_lock=0
stale_locked=0
hid_idle_s() {
...
pending_lock() {
    ...
    elif [ "${stale_locked}" = 0 ] && [ "${last_sample}" != 0 ] \
        && [ "$((now - last_sample))" -ge "${SILENT_NOTICE_S}" ]; then
