...
- 已启动并验证能引导：进程稳定，客户机拿到了 IP（`tart ip repose-spike` 查）
...
| 服务条款 | 同意 | |
| 账户 | 名称 `admin`，密码自己定一个 | **必须记住**，后面 ssh 和 sudo 都要用；`vm-env.sh` 默认这个用户名 |
| 定位服务 | 关 | |
| 分析 / Siri / 屏幕使用时间 | 全部跳过 | |
| Touch ID | 跳过 | |
| 外观 | 随意 | |

密码可以用弱的：这台 VM 会被反复快照回滚，不存任何真实数据，而实验里要频繁
输密码验证 fallback。别用你真在用的密码，也别把它写进仓库。
