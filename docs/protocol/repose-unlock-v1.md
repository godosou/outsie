...
The phone MUST durably coordinate response allocation for each `(MacId, DeviceId, pairing_generation)` pairing. Its durable record is either ready with a counter, or cached with that counter plus the complete `SessionBinding`, `ChallengeId`, `SHA-256(C)` fingerprint of the exact signed 249-byte Challenge frame, and exact 390-byte Response frame. Response construction consumes the opaque authenticated-Challenge capability and allocates `max(durable_phone_counter, counter_floor) + 1` using checked arithmetic. Before CAS, the phone MUST parse the signer's raw low-S output and verify the complete Response signature with the phone identity public key carried from its local paired-Mac record through that opaque capability; neither the Challenge, Response, nor signer selects this key. Cached and concurrent-winner Responses MUST undergo the same signature check against the exact Challenge transcript and paired key before return. The phone MUST atomically compare-and-swap the complete cached record and make it durable before returning the Response. Any record obtained through `load` is only a candidate, never authorization to return its bytes: every cached hit and concurrent-winner candidate MUST first pass an exact expected-to-same-expected CAS against authoritative durable state. This self-CAS is a real durable comparison in the same linearization domain as allocation and pairing rotation, even though it does not change bytes; it MUST NOT be short-circuited as a no-op or answered from a cache. A storage error, failed confirmation, failed allocation CAS without a confirmed fully matching winner, signing or signature-validation failure, randomness failure, overflow, or failed consistency read-back returns no candidate Response.

An exact retransmission of the current signed Challenge returns the exact cached Response without invoking randomness or the phone signer and without advancing the counter, but only after the atomic self-CAS confirms that exact loaded record is still authoritative. Within one coordinator, a non-poisoning local mutex also prevents duplicate candidate generation. Distinct coordinators or processes racing from the same durable expected state may each compute a different randomized candidate, but exactly one durable CAS may advance the counter; every successful caller MUST return the exact durable winner bytes, never its losing candidate. A CAS loser accepts a loaded winner only when Mac ID, device ID, generation, binding, ChallengeId, Challenge fingerprint, expected next counter, mirrored Challenge fields, structural cache invariant, and paired-key signature all match its allocation intent and an exact self-CAS confirms that record is authoritative. The confirmation CAS is the return linearization point: if a pairing rotation linearizes first, confirmation fails; if confirmation linearizes first, that Response was authoritative at its return decision. A fresh allocation winner is likewise self-CAS-confirmed before release; its subsequent load is consistency diagnosis only and never independently authorizes a Response. Within one lock epoch, a lower `ChallengeId` is stale and the same `ChallengeId` with any different signed frame, nonce, binding, or other fingerprint input is a conflict. A strictly higher lock epoch may restart the reducer `ChallengeId` sequence, but a full Mac unlock-service restart MUST mint a strictly higher `lock_epoch`; reuse of the same epoch/ID fails closed. Initial pairing and every pairing-generation change require an explicit durable phone-side pairing rotation/reset operation; missing state and response-time generation changes fail closed, so loss of the counter record cannot silently reset it. The durable cache and exact Response bytes survive ordinary phone-process restart.
...
The reducer may then emit one non-Clone/non-Copy linear permit command. A permit binds the complete `SessionBinding`, ChallengeId, device identity, pairing generation, durable counter, and originating replay guard, and uses only Mac-local creation/expiry times. Public consumption requires that same replay guard as a sealed generation authority. Under the permit store's non-poisoning lock it checks time, the authoritative current session, and atomically compares the exact expected `Active` record with durable current state before removing and returning the permit. This compare-current operation is an exact expected-to-same-expected CAS in the same linearization domain as revocation; a storage adapter MUST perform the durable comparison even though the replacement bytes equal the expected bytes and MUST NOT short-circuit it from a stale cache. A later load may classify rejection but can never authorize after the atomic comparison failed. An unavailable store, wrong guard, changed durable state, or matching/later revoked tombstone yields no consumed capability and destroys the permit. Thus every consume invoked after a revoke has returned fails even if an ordinary read can be stale. For genuinely overlapping consume/revoke calls, the authority self-CAS and revoke CAS are the ordering points: a consume whose self-CAS wins may succeed; one ordered after the revoke cannot.
