...
1. **多把钥匙时状态信标怎么广播。** `macstate` 的 tag 是用某一把 K 算的。
   Mac 只有一把「自己这台对这部手机」的钥匙，所以对每部手机各广播一路——
   和手机那边对称。两部手机时 Mac 要开两路广播，macOS 的 CoreBluetooth
   peripheral 能不能同时开多路，**没验过**。
2. **校准是每部手机一份。** `calibration.json` 改成按 keyId 索引的字典，
   bridge 按行里的 keyId 选阈值。
