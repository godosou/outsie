...
export function ShortcutsPanel({ bridge, onToast }: {
  ...
  return (
    <>
      {/* The other way to fill this page: hand the job to an AI. The text it
          needs -- the command, the spelling of keys, what is already here --
          comes from the same code the CLI prints, so it is never out of date
          (design doc §07). */}
      <section className="panel preferences-panel">
        <div className="section-heading">
          <div>
            <h2>让 AI 帮你配</h2>
            <p>把说明复制给任何会跑命令的 AI（Claude Code、Codex…），说一句「给飞书加个搜索」，它就用 <code>outsie shortcuts</code> 改好；你在手机上同步一次。</p>
          </div>
          <button
            className="button light"
            onClick={() => {
              void bridge.aiPrompt().then(text => navigator.clipboard.writeText(text)).then(
                () => onToast?.('复制好了。贴给 AI，说你要什么按钮。'),
                () => onToast?.('没复制成。终端里跑 outsie shortcuts --prompt 也能拿到同一段。'),
              )
            }}
          >
            复制给 AI 的说明
