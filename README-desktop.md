# Outsie Electron 0.1.1 桌面版（基线）

当前 Rust / Tauri 分支请阅读 [Rust 版说明](README-rust.md)。本文件记录 `main` 分支的 Electron 实现。

Outsie（歇一会）提供 Electron 桌面运行方式，使用与网页预览相同的界面。关闭主窗口后，应用继续留在菜单栏／系统托盘，休息计时继续运行。需要彻底退出时，选择托盘菜单中的「退出 Outsie」。
...
开发时运行 `npm run desktop:dev`。启动脚本会在 `127.0.0.1:47832` 启动 Vite，等待服务就绪后打开 Electron。若该地址已有 Outsie 的 Vite 开发服务，则直接复用；退出桌面进程时，只关闭脚本自行启动的服务。

首次启动 Electron 需要下载对应操作系统的运行文件。如果下载未完成，请恢复网络后运行 `npx install-electron --no`，再重试。运行 `npm run package:mac` 可生成自带运行环境的 `release/Outsie.app`，无需通过命令行启动。当前采用本机 ad hoc 签名，尚未做 Apple Developer 签名或公证，也未包含自动更新和开机启动设置。
...
- 同一用户只运行一个 Outsie 实例。再次启动会显示已有窗口。
- 桌面数据保存在 Electron 的 `Repose` 用户数据目录（沿用改名前的目录名，避免升级后丢失已有设置与记录），与浏览器预览数据相互独立。
...
旧版系统存在 `CGSession` 时使用 `CGSession -suspend`。当前 macOS 没有该程序时，通过系统事件发送 Apple 官方的 Control-Command-Q 锁屏快捷键，需要授予 Outsie「辅助功能」和相应「自动化 → 系统事件」权限。在偏好设置中可打开系统辅助功能设置。应用会检查系统锁屏事件；命令失败或未确认实际锁定时，界面会显示错误并关闭该开关，授权后可重新启用。
