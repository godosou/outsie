...
pub enum Command {
    ...
    Schema,
    Examples,
    /// The text to paste into an AI; see [ai_prompt].
    Prompt,
    Help,
}
...
pub fn apply(config: &mut ConsoleConfig, cmd: &Command) -> Result<String, String> {
    let msg = match cmd {
        ...
        Command::List { .. } | Command::Apps | Command::Export | Command::Schema | Command::Examples | Command::Prompt | Command::Help => {
...
pub fn writes(cmd: &Command) -> bool {
    !matches!(cmd, Command::List { .. } | Command::Apps | Command::Export | Command::Schema | Command::Examples | Command::Prompt | Command::Help)
}
...
pub fn render(config: &ConsoleConfig, cmd: &Command) -> String {
    match cmd {
        ...
        Command::Prompt => ai_prompt(config),
...
/// What a person pastes into an AI so it can set the buttons up for them
/// (design doc §07). Self-contained: the command, the spelling of keys, the
/// import format, what is already there, and the two rules that keep the
/// phone right -- names are the merge key, and the phone syncs at the end.
pub fn ai_prompt(config: &ConsoleConfig) -> String {
    let mut have = String::new();
    for app in &config.apps {
        have.push_str(&format!("- {}（{}）：{} 个操作", app.name, app.bundle_id, app.actions.len()));
        if !app.actions.is_empty() {
            let names: Vec<&str> = app.actions.iter().take(8).map(|a| a.name.as_str()).collect();
            have.push_str(&format!("，已有 {}{}", names.join("、"), if app.actions.len() > 8 { "…" } else { "" }));
        }
        have.push('\n');
    }
    if have.is_empty() {
        have.push_str("- （还没有任何 App）\n");
    }
    format!(
"你是我的 Mac 上 Outsie「快捷控制」的配置助手。我在手机上点一个 App 的名字，Mac 会切到那个 App；点一个按钮，Mac 切过去再替我按那组键。配置只能通过这条命令改：

    outsie shortcuts

先跑 `outsie shortcuts list` 看现状，再改。改的方式是准备一份 JSON 交给 `outsie shortcuts import 文件.json`（或 `import -` 读标准输入）。格式：

    {{ \"apps\": [ {{ \"name\": \"飞书\", \"bundleId\": \"com.electron.lark\", \"actions\": [
        {{ \"name\": \"搜索\", \"keys\": \"cmd+k\", \"icon\": \"⌕\" }},
        {{ \"name\": \"左右分屏\", \"keys\": \"ctrl+b, %\" }} ] }} ] }}

规则：
1. 按键这样写：cmd+k、shift+cmd+p、alt+cmd+right、enter、esc、tab、space、up、down、f12；几步用逗号隔开（ctrl+b, %）。修饰键只有 cmd、ctrl、alt、shift。
2. App 的 name 和操作的 name 是合并的依据：同名 App 只更新，同名操作只换按键，按钮不会换位。新 App 必须给 bundleId（`mdfind \"kMDItemCFBundleIdentifier == 'com.x.y'\"` 或 `osascript -e 'id of app \"名字\"'` 能查）。
3. 操作名要短，是人在手机上一眼认得的动作，不带 App 名。icon 一个字符，可省。
4. 只加我要的，不要删我没提的。删用 `outsie shortcuts remove --app X --name Y`。
5. 改完跑一次 `outsie shortcuts list` 确认，然后提醒我：在手机上打开控制页按一次「同步」。

现在已有：
{have}
`outsie shortcuts --schema` 是格式的 JSON Schema，`outsie shortcuts --examples` 有更多例子。",
        have = have.trim_end(),
    )
}
...
pub const HELP: &str = "outsie shortcuts — 从命令行看和改快捷控制的列表
  ...
  outsie shortcuts --prompt                         整段复制给 AI 的说明（快捷键设置页上也有）
...
mod tests {
    ...
    #[test]
    fn the_ai_prompt_carries_everything_an_agent_needs_and_what_is_already_there() {
        let c = cfg();
        let p = ai_prompt(&c);
        for must in ["outsie shortcuts list", "outsie shortcuts import", "\"keys\"", "cmd+k", "ctrl+b, %", "bundleId", "同步", "飞书", "com.electron.lark", "搜索", "--schema"] {
            assert!(p.contains(must), "missing {must:?}");
        }
        // The example JSON in the prompt is the import format, so an agent that
        // copies it verbatim gets a valid document.
        let start = p.find("{ \"apps\"").expect("an example document");
        let end = p[start..].find("] } ] }").expect("closed") + start + "] } ] }".len();
        let mut fresh = ConsoleConfig::default();
        apply(&mut fresh, &Command::Import { json: p[start..end].to_string() }).expect("the example imports");
        assert_eq!(fresh.apps[0].actions.len(), 2);
        // Empty config: says so rather than listing nothing.
        assert!(ai_prompt(&ConsoleConfig::default()).contains("还没有任何 App"));
        assert_eq!(render(&c, &Command::Prompt), p);
        assert!(!writes(&Command::Prompt));
    }
}
