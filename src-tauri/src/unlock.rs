...
pub(crate) fn resolve_ble_dir(app: &AppHandle) -> Option<PathBuf> {
    ...
    // The checkout fallback exists for `tauri dev` only. A release build must
    // not carry the build machine's path, so it is compiled out there.
    #[cfg(debug_assertions)]
    {
        let dev = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../tools/ble-spike/mac");
        if dev.join("presence-pipeline.sh").exists() {
            return Some(dev);
        }
    }
    None
...
fn resolve_scripts_dir(app: &AppHandle) -> Option<PathBuf> {
    ...
    // Dev fallback: <crate>/../native/macos/minimal-auth-plugin. Debug builds
    // only, so a release binary never carries the build machine's path.
    #[cfg(debug_assertions)]
    {
