...
pub fn issue_challenge<R: CryptoRandom>(
    ...
    let frame = wire::encode_challenge(&message);
    Ok(IssuedChallenge {
        message,
        frame,
...
impl SessionMaterial {
    ...
    #[must_use]
    pub const fn mac_to_phone_key(&self) -> &[u8; 32] {
        &self.mac_to_phone_key
    }

    #[must_use]
    pub const fn mac_to_phone_nonce(&self) -> &[u8; 12] {
        &self.mac_to_phone_nonce
    }

    #[must_use]
    pub const fn phone_to_mac_key(&self) -> &[u8; 32] {
        &self.phone_to_mac_key
    }

    #[must_use]
    pub const fn phone_to_mac_nonce(&self) -> &[u8; 12] {
...
pub fn derive_session_material(
    ...
    let shared = diffie_hellman(private_key.to_nonzero_scalar(), phone_public.as_affine());
    let shared_secret: [u8; 32] =
        shared
            ...
            .try_into()
            .map_err(|_| {
                // P-256 always returns exactly one 32-byte field element.
                CryptoError::InvalidPhonePublicKey
            })?;
    let context = transcript::context_hash(&issued.frame, response);
...
#[must_use]
pub fn kdf_context_hash(issued: &IssuedChallenge, response: &Response) -> [u8; 32] {
    transcript::context_hash(&issued.frame, response)
}

#[must_use]
pub fn hkdf_salt(issued: &IssuedChallenge, response: &Response) -> [u8; 32] {
    transcript::salt(&kdf_context_hash(issued, response))
}

#[must_use]
pub fn response_aad(issued: &IssuedChallenge, response: &Response) -> Vec<u8> {
    transcript::aad(&issued.frame, response)
}

#[must_use]
pub fn response_plaintext(issued: &IssuedChallenge, response: &Response) -> [u8; 32] {
    transcript::proof(&kdf_context_hash(issued, response))
}

#[must_use]
pub fn signature_transcript_hash(issued: &IssuedChallenge, response: &Response) -> [u8; 32] {
    transcript::signature_hash(&issued.frame, response)
}

pub fn decrypt_response_proof(
    issued: &IssuedChallenge,
    response: &Response,
    ...
    let expected = response_plaintext(issued, response);
    if plaintext.as_ref().ct_eq(&expected).unwrap_u8() != 1 {
...
pub fn verify_response(
    ...
    verifying_key
        .verify_prehash(&signature_transcript_hash(issued, &response), &signature)
        .map_err(|_| VerificationError::Signature)?;
